 ...
 * @version $Id: function.addthis_widget_drop.php 164 2011-03-25 09:39:42Z kevin $
 ...
 * 'link'     = the URL to be saved (if passed, 'item' will be ignored)
 * 'item'     = the URL to be saved
 * 'title     = name for the link
 */
function smarty_function_addthis_widget_drop($params, &$smarty)
{
	$item = isset($params['item']) ? $params['item'] : $GLOBALS['MENU'];
	$uid = isset($params['username']) ? $params['username'] : "";
	$link = isset($params['link']) ? $params['link'] : LinkHelper::itemUrl( $item );
